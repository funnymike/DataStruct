...
class DoubleLinkedList: LinkListType {
    ...
    func insertItem(item: AnyObject, index: UInt) -> Bool {
        ...
        let newItem = DoublyLinkedListNote(data: item)
        newItem.next = cursor?.next
        if cursor?.next != nil {
            cursor?.next?.pre = newItem
        }
        cursor?.next = newItem
        newItem.pre = cursor
